...
class AuthenticationModel:
    ...
    def _register_credentials(self, username: str, first_name: str, last_name: str,
        ...
        """
        Adds the new user's information to the credentials dictionary and the database.
        ...
        """
        # Saving user data in the credentials dictionary
        ...
        # Connecting to the database and inserting user data
        ...
        cursor = database_connection.execute("""
            INSERT INTO users (username, failed_login_attempts, first_name, last_name, logged_in, email, password, password_hint, roles, theme_button, current_chat, selected_model)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (username, 0, first_name, last_name, 0, email, Hasher.hash(password), password_hint, ','.join(roles) if roles else NULL_value, NULL_value, NULL_value, NULL_value))
    ...
    def register_user(self, new_first_name: str, new_last_name: str, new_email: str,
        ...
        if self._credentials_contains_value(new_email):
            raise RegisterError('Почта уже занята')
        if new_username in self.credentials['usernames']:
            raise RegisterError('Имя пользователя/почта уже заняты')
