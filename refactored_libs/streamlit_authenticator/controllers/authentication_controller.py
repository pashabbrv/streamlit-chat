...
class AuthenticationController:
    ...
    def register_user(self, new_first_name: str, new_last_name: str, new_email: str,
        ...
        if not self.validator.validate_name(new_first_name):
            raise RegisterError('Имя введено некорректно.')
        if not self.validator.validate_name(new_last_name):
            raise RegisterError('Фамилия введена некорректно.')
        if not self.validator.validate_email(new_email):
            raise RegisterError('Почта введена некорректно.')
        if domains:
            if new_email.split('@')[1] not in ' '.join(domains):
                raise RegisterError("Почта введена в некорректном формате, поэтому не может быть зарегистирована. Проверьте наличия символа '@'.")
        if not self.validator.validate_username(new_username):
            raise RegisterError('Имя пользователя введено некорректно.')
        if not self.validator.validate_length(new_password, 1) \
            or not self.validator.validate_length(new_password_repeat, 1):
            raise RegisterError('Поля пароля/его повтора не могут быть пустыми.')
        if new_password != new_password_repeat:
            raise RegisterError('Пароли не совпадают.')
        if not self.validator.validate_length(password_hint, 1):
            raise RegisterError('Подсказка для пароля не может быть пустой.')
        if not self.validator.validate_password(new_password):
            raise RegisterError('Пароль не соответствует критериям: Длина от 8 до 20 символов. Содержит как минимум одну строчную букву. Содержит как минимум одну заглавную букву. Содержит как минимум одну цифру. Содержит как минимум один специальный символ из [@$!%*?&].')
        if roles and not isinstance(roles, list):
            raise LoginError('Роли должны передаваться в качестве списка.')
        if captcha:
            if not entered_captcha:
                raise RegisterError('Капча не введена.')
            entered_captcha = entered_captcha.strip()
